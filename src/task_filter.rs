...
impl Task {
    fn new(tasks: &Option<String>) -> Self {
        let tasks = if let Some(tasks) = tasks {
            tasks.split('.').rev().map(String::from).collect()
        } else {
            vec![]
        };
...
impl TaskFilter {
    pub fn new() -> Self {
        Self { taskset_first: Task::new(&None), taskset_last: Task::new(&None) }
    }
    ...
    pub fn taskset_interval(&mut self, first: &Option<String>, last: &Option<String>) {
        self.taskset_first = Task::new(first);
        self.taskset_last = Task::new(last);
    }
    ...
    pub fn filter_layers(&self, layers: &Vec<Vec<String>>) -> Vec<Vec<String>> {
        ...
        for layer in layers {
                ...
                && layer.contains(first)
            {
                ...
                {
                    if first == last {
                        new_layers.push(vec![first.to_string()]);
                    } else {
                        new_layers.push(vec![first.to_string(), last.to_string()]);
                    }
                    break;
                } else {
                    new_layers.push(vec![first.to_string()]);
            ...
            if let Some(last) = last
                && layer.contains(last)
            {
                new_layers.push(vec![last.to_string()]);
                break;
            }

            if get {
                new_layers.push(layer.to_owned());
            }
        }
